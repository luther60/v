...
if(isset($_GET['id_car'])) { 
  $vehicule = getVehiculeById($pdo, $_GET['id_car']);?>
   ...
   <h1 class="title_index">L'action est irréversible, en cliquant sur "supprimer définitivement" , vous allez être redirigé.</h1>
    <ul class="recap">
      <li class="title_index"><?=$vehicule['marque']?></li>
      <li class="title_index"><?=$vehicule['modele']?></li>
      <li class="title_index"><?=$vehicule['annee']?></li>
      <li class="title_index"><?=$vehicule['prix']?></li>
    </ul>
    <div class="call_car"><a  href="finalDelete.php?id_car=<?=$_GET['id_car'];?>">Supprimer définitivement</a></div>
<?php } ?>
